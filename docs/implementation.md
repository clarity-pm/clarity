...
Clarity is a lightweight work management framework and requires relatively minimal effort to implement. However,
...
For production organizations (e.g., manufacturing, software, product), Clarity prefers that the organization, at its lowest
level, be divided into one or more performance production teams. Such teams should be of limited size, cross-functional,
semi or fully self-managing, and fairly autonomous with strong
...
cross-functional and the work is a mix of several objectives (e.g., “classic” software development agencies), doing so
will not be as efficient.

For service organizations (e.g., logistics, retail), it is also possible that no such logical distinction exists and that
all teams, except specific departments, or even most of the staff, are fairly equal in terms of the area of
...
boundaries are set artificially. This will help further subdivide work and smaller units are easier to manage long
term.

It is perfectly fine to apply the Clarity framework selectively, meaning that the entire organization does not need to
operate under the Clarity framework. It is possible that some part of the staff has no benefit from using Clarity, as rare
as these cases might be - they exist.

Clarity also does not require organization-wide application from day one. Starting the implementation from one or
more pilot teams is just fine, just to validate the applicability of the framework to your particular use case. Despite
that Clarity is designed to be fairly universal and industry-agnostic, you might still encounter roadblocks that
...
1. Identify the current status of your organization. Write down how things are organized, who is responsible for what areas
   of the organization, and why. Identify stakeholders.
...
5. Have a conversation with all stakeholders that would be involved in the transition. Determine and install
   countermeasures for all blockers and roadblocks identified.
6. Establish a pilot - a single unit or team to adopt Clarity before everyone else.
7. Learn and educate! Work with your pilot team to implement Clarity for at least 6 delivery cycles.
8. Evaluate the results of the pilot - did you achieve your goals? What did you learn? Are there any adjustments that need
   to be made before moving on?
9. Rinse and repeat - for each unit or team individually until Clarity is implemented to the extent necessary.
...
You should be able to use any task or project management software to implement Clarity as long as it has the following
features available:

- Tasks can be visualized and are organized as lists; Kanban-only tools will not do.
- Tasks in these lists can be separated into sections of tasks to support [stages](concepts/task-stage.md).
...
| Name        | URL                    | Notes                                                                                                                                                            |
|----------	|----------------------	|--------------------------------------------------------------------------------------------------------------------------------------------------------------	|
| ClickUp    | https://clickup.com    | Clarity can be implemented using the free version. All-in-one project management solution.                                                                        |
| Asana        | https://asana.com        | Clarity can be implemented using the free version. For best results, the paid version is required to use custom fields. Powerful customization options are available.    |
...
### What about Atlassian JIRA?

Atlassian JIRA can be adapted for Clarity, but doing so requires some workarounds and customization - you will need to
define custom statuses, custom priorities, and custom labels, and rename components like Epics and such, to match the Clarity
framework. JIRA has no support for custom sections in backlogs, so you might need to use Sprints to support task Stages.
This is obviously a hack. If JIRA is all that you can have or are allowed to have, you can make it work, but the process
